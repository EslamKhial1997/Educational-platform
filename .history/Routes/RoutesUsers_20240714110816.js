...
Routes.route("/").post(uploadImage,createUsersValidator,resizeImage("user"), createUsers).get(getUsers);
Routes.route("/verifycode").post(verifyRegister);
Routes.route("/addpoint/:id").put(updateUserPoint);
Routes.route("/:id").get(getOneUserValidator, getUser).delete(allowedTo( "manager"),deleteOneUserValidator, deleteUser)
.put(uploadImage,createUsersValidator, updateOneUserValidator, updateUser)
module.exports = Routes;resizeImage("user"),
