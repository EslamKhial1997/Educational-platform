...
Routes.route("/")
  .post(allowedTo("admin", "manager"),uploadImage, createClassValidator, resizeImage("class"), createClasses)
  .get(getClassess);
Routes.route("/:id")
  .get(getClassValidator, getClass)
  .put(allowedTo("admin", "manager"),uploadImage, updateClassValidator, resizeImage("class"), updateClass)
