...
exports.createUsersValidator = [
  check("username")
    .notEmpty()
    .withMessage("is required UserName")
    .custom((val, { req }) => {
  ...
  check("guardianPhone")
    .notEmpty()
    .isMobilePhone(["ar-EG", "ar-SA"])
    .withMessage("Invailable phone number EG , SA Number Only accepted").custom((val) =>
      createUsersModel.findOne({ guardianPhone: val }).then((user) => {
        if (user) {
          return Promise.reject(new Error("guardianPhone Is Already in Used"));
        }
      })
    ),
  MiddlewareValidator,
