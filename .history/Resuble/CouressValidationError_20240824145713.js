...
exports.createCourseValidator = [
  ...
  check("coupon")
    ...
    .custom(async (val, { req }) => {
      ...
      if (!couponModel) {
        return Promise.reject(new Error(`الكوبون ${req.body.coupon} غير صالح أو منتهي`));
      }
  ...
  check("lacture")
    .optional()
    .custom(async (val, { req }) => {
      const lectureModel = await createLecturesModel.findOne({ _id: req.body.lacture });
      if (!lectureModel) {
        return Promise.reject(new Error(`المحاضرة غير موجودة`));
      }

      // التحقق مما إذا كان الكوبون ينتمي لهذه المحاضرة
      if (req.couponModel && val !== req.couponModel.lecture._id.toString()) {
  ...
  check("section")
    .optional()
    .custom(async (val, { req }) => {
      const sectionModel = await createSectionModel.findOne({ _id: req.body.section });
      if (!sectionModel) {
        return Promise.reject(new Error(`الباب غير موجود`));
      }

      // التحقق مما إذا كان الكوبون ينتمي لهذا القسم
      if (req.couponModel && val !== req.couponModel.section._id.toString()) {
        return Promise.reject(
