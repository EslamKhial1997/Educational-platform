...
exports.UpdateUserPassword = [
  ...
  check("password")
    ...
    .custom(async (val, { req }) => {
      const user = await createUsersModel.findById(req.user.id);

      if (!user) {
        throw new ApiError("الباسورد القديم غير صحيح", 500);
      }
      ...
      if (val !== req.body.passwordConfirm) {
        throw new ApiError("in Correct passwordConfirm", 500);
      }
    }),
  MiddlewareValidator,
];
