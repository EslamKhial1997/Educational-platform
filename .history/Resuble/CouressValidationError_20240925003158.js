...
exports.createCourseValidator = [
  check("coupon")
    ...
    .custom(async (val, { req }) => {
      const couponModel = await createCouresModel.findOneAndUpdate(
        { user: req.user._id },
        {
  ...
  check("lacture")
    ...
    .custom(async (val, { req }) => {
      ...
      if (!lectureModel) {
        createCouresModel.findOneAndUpdate(
          { user: req.user._id },
          {
            $pull: { couresItems: { _id: req.params.id } },
          },
      ...
      if (req.couponModel && req.couponModel.lecture) {
        console.log(req.couponModel.lecture._id.toString(), req.body.lacture);
        if (req.couponModel.lecture._id.toString() !== req.body.lacture) {
          return Promise.reject(
