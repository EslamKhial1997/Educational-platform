...
exports.UpdatePaidToTeacherValidation = [
  check("id")
    ...
    .withMessage("في المدرس غير موجود"),
  check("point").isEmpty()
    .withMessage("النقاط مطلوبه")
    .custom((val) =>
      createTeachersModel.findOne({ point: val }).then((teacher) => {
        if (!teacher) {
          return Promise.reject(new Error(" النقاط المطلوب خصمها اكبر من نقاط المدرس"));
        }
      })
    )
    .withMessage("في المدرس غير موجود"),
