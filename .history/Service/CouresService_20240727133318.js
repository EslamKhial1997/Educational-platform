...
exports.createCoures = expressAsyncHandler(async (req, res, next) => {
  ...
  if (!coures) {
    cart = await createCouresModel.create({
      user: req.user._id,
      couresItems: [{ lacture, lacture }],
    });
  } else {
    const couresExists = coures.couresItems.findIndex(
      (item) =>
        item.lacture._id.toString() === lacture.toString() 
    ...
    if (couresExists > -1) {
      ...
      await coures.couresItems.push({ lacture, color, price: productModel.price });
    }
  }
