...
exports.deleteOne = (Model) =>
  expressAsyncHandler(async (req, res, next) => {
   ...
   const baseUrl = `${process.env.BASE_URL}/teacher`;
      const relativePathimage = url.replace(baseUrl, "" + deleteDoc.image);
      const relativePathPicture = url.replace(baseUrl, "" + deleteDoc.picture);
      const relativePathAvatar = url.replace(baseUrl, "" + deleteDoc.avater);
    if (!deleteDoc) {
      return next(
    ...
    deleteFile(path.resolve(category.avatar));
    // try {
    //   const baseUrl = `${process.env.BASE_URL}/teacher`;
