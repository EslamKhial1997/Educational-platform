...
exports.getTransactionsYear = expressAsyncHandler(async (req, res, next) => {
  ...
  const totalPoints = await createTransactionModel.aggregate([
    {
      $match: {
        receiver:Mongoose.Types.ObjectId(receiver)  // استبدل هذا بالـ ID الخاص بالمستقبل
      }
    },
    {
      $group: {
        _id: null, // يمكنك تعديل هذا إذا كنت ترغب في تجميع البيانات بناءً على معيار معين
        totalPoints: { $sum: "$pointsSent" } // جمع النقاط المرسلة
      }
    }
    // { $group: { _id: null, total: { $sum: "$pointsSent" } } },
  ]);
//  const totalPoints = await createTransactionModel.find({
//     receiver: receiver
//   })
console.log(receiver ,totalPoints);
  res.send({
    transactions,
    totalPoints: totalPoints.length > 0 ? totalPoints[0].total : 0,
  });
