...
const createTeachersModel = require("../Modules/createUsers");
...
exports.createTeachers = expressAsyncHandler(async (req, res) => {
  ...
  const user = await createTeachersModel.create(req.body);
...
exports.verifyRegister = expressAsyncHandler(async (req, res, next) => {
  ...
  const user = await createTeachersModel.findOne({
    code: ciphertext,
...
exports.getUsers = factory.getAll(createTeachersModel);
exports.getUser = factory.getOne(createTeachersModel);
exports.updateUserPoint = expressAsyncHandler(async (req, res, next) => {
  ...
  const userLogged = await createTeachersModel.findById(req.user._id);
...
// exports.deleteUser = factory.deleteOne(createTeachersModel);
...
exports.updateLoggedUserPassword = expressAsyncHandler(async (req, res) => {
  const user = await createTeachersModel.findByIdAndUpdate(
    req.user._id,
    {
...
//   const updatedUser = await createTeachersModel.findByIdAndUpdate(
//     req.user._id,
//     {
//       name: req.body.name,
...
//   await createTeachersModel.findByIdAndUpdate(req.user._id, { active: false });
