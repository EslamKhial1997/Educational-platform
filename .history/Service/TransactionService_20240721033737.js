...
exports.getTransactionsYear = expressAsyncHandler(async (req, res, next) => {
  ...
  const endDate = new Date(year, 11, 31, 23, 59, 59); // نهاية آخر يوم في السنة
  ...
  if (!transactions.length) {
    return res.status(404).send({ message: 'No transactions found for the specified year and receiver.' });
  }

  const totalPoints = await createTransactionModel.aggregate([
    { $match: { receiver: receiver, date: { $gte: startDate, $lt: endDate } } },
    { $group: { _id: null, total: { $sum: '$pointsSent' } } }
  ]);
