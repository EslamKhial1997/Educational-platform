...
exports.restNewPassword = (UserPassword) =>
  expressAsyncHandler(async (req, res, next) => {
    ...
    if (!user && !teacher) {
      return next(
        new ApiError(`لايوجد ايميل بهذا الاسم ${email}`, 404)
      );
    }
    ...
    if (!target.userVerify) {
      return next(new ApiError("لم يتم التحقق من رمز إعادة الضبط", 400));
    }
    ...
    res.status(200).json({ status: "success" , msg:"تم تغيير الرقم السري بنجاح" });
  });
