...
console.log(lactureModel.price , req.user.point);
  // Check if the lactureModel.price > req.user.point
  if (lactureModel.price > req.user.point) {
    return next(new ApiError(`Price Lecture: ${lactureModel.price} > Your Point ${req.user.point}`, 500));
  }
  ...
  if (!coures) {
    ...
    if (couresExists > -1) {
      ...
      console.log(lacture, couponModel);
      await coures.couresItems.push({ lacture, couponModel });
    }
  }
