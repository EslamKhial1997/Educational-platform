...
const createDigetsCodeModel = require("../Modules/digetsCode");
const expressAsyncHandler = require("express-async-handler");
...
exports.createUsers = expressAsyncHandler(async (req, res) => {
  ...
  const user = createDigetsCodeModel.create({
  ...
  const hashed = crypto.createHash("sha256").update(restcode).digest("hex");
  const userExist = await createDigetsCodeModel.findOne({
    code: hashed,
  ...
  await createUsersModel.create(req.body);
  res.status(200).json({ status: "success", data: userExist });
...
exports.digetsCodeSend = expressAsyncHandler(async (req, res, next) => {});
