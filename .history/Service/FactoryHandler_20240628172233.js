...
exports.getOne = (Model, populateOpt) =>
  ...
  const deleteFiles = async (filePaths) => {
    try {
      // استخدام Promise.all لتنفيذ عمليات الحذف بشكل متزامن
      await Promise.all(filePaths.map(filePath => fs.unlink(filePath)));
      console.log('تم حذف جميع الملفات بنجاح');
    } catch (err) {
      console.error('No Deletes', err);
    }
  };
exports.deleteOne = (Model) =>
  expressAsyncHandler(async (req, res, next) => {
    ...
    const filePaths = [
      path.resolve(filePathImage),
      path.resolve(filePathPicture),
      path.resolve(filePathAvater)
    ];
