...
exports.deleteOne = (Model) =>
  expressAsyncHandler(async (req, res, next) => {
   ...
   const baseUrl = `${process.env.BASE_URL}/teacher/`;
    ...
    console.log(deleteDoc.image ,deleteDoc.picture ,deleteDoc.avater ,relativePathimage);
 

    //   // حذف الصورة من ملفات المشروع
     const filePathImage = path.join(__dirname, "../uploads/teacher", relativePathimage);
     const filePathPicture = path.join(__dirname, "../uploads/teacher", relativePathPicture);
     const filePathAvater = path.join(__dirname, "../uploads/teacher", relativePathAvatar);
    //   console.log(filePath);
    try{
      fs.unlink(filePathImage ,filePathPicture ,filePathAvater, (err) => {
        if (err) {
      ...
      console.error("حدث خطأ أثناء الحذ", error);
    }
    // deleteDoc.remove();
