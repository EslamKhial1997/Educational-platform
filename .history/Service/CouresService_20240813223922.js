    ...
    if (!coures) {
      ...
      coures = await createCouresModel.create({
        ...
        couresItems: [
          {
            ip: serverIp,
            lacture: lactureModel ? req.body.lacture : null,
            section: sectionModel ? req.body.section : null,
            teacherID: sectionModel.class.teacher._id,
            coupon: couponModel ? couponModel.code : null,
            discount: couponModel ? couponModel.discount : null,
          },
      ...
      // التحقق إذا كانت المحاضرة أو الفصل موجود بالفعل في الدورة
      const courseExistsIndex = coures.couresItems.findIndex(
        (item) =>
          (lactureModel && item.lacture?.toString() === req.body.lacture?.toString()) ||
          (sectionModel && item.section?.toString() === req.body.section?.toString())
      );

      if (courseExistsIndex === -1) {
        // إذا لم تكن المحاضرة أو الفصل موجود، أضفهم إلى الدورة
        coures.couresItems.push({
          lacture: lactureModel ? req.body.lacture : null,
          section: sectionModel ? req.body.section : null,
          teacherID: sectionModel.class.teacher._id,
        ...
        return res.status(400).json({
          status: "Failure",
          message: "Lecture or Section already exists in the course. No update needed.",
        });
      }
    }
