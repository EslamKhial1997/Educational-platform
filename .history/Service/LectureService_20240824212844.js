...
exports.createLectures = expressAsyncHandler(async (req, res) => {
  try {
    const section = await createSectionModel.findById(req.body.section).then((e) => e.class.teacher._id);
    const teacher = await createTeachersModel.findById(section).then((e) => {})
    console.log(teacher.accessKey);
