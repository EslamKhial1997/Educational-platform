...
exports.createFirstManagerAccount = async () => {
  const manager = await createUsersModel.findOneAndUpdate(
    { email: "manager@gmail.com" },  // البحث عن حساب باستخدام البريد الإلكتروني
    {
      name: "manager",
    ...
    { upsert: true, new: true }  // upsert ينشئ سجلًا جديدًا إذا لم يكن موجودًا
  );

  console.log("Manager account created or updated successfully");
};
