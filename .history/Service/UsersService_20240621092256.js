...
exports.updateUserPoint = expressAsyncHandler(async (req, res, next) => {
  ...
  if (req.user.role === "manager" && req.user.point < req.body.point) {
    return Promise.reject(new Error(`Sorry Your Points ${req.user.point} > ${req.body.point}`));
  }
 ...
 const pluspoint =  updateDocById.point;
 pluspoint  = updateDocById.point + req.body.point;
