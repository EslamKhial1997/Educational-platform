...
exports.createUsers = expressAsyncHandler(async (req, res) => {
  ...
  //     text: `Your verification code is: ${ciphertext}`,
  //   };
  ...
  try {
    ...
    // حفظ الرمز مؤقتاً في ذاكرة الخادم
    // في تطبيق حقيقي، يجب استخدام آلية تخزين مؤقتة مثل Redis
    await redisClient.setEx(req.body.email, 600, digitCode); 
    // await client.set('code', req.body.email);
...
exports.verifyCode = expressAsyncHandler(async (req, res, next) => {
    ...
    .on('error', err => console.log('Redis Client Error', err))
    .connect();
  redisClient.get(req.body.email, async (err, storedCode) => {
    ...
    //     // إزالة الرمز المؤقت من Redis
    //     redisClient.del(email);

    //     res.status(200).send('User added successfully');
    //   } catch (error) {
    //     console.error('Error adding user to database:', error);
    //     res.status(500).send('Failed to add user to database');
    //   } finally {
    //     await client.close();
    //   }
    // } else {
