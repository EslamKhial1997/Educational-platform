...
exports.updateSpecificCourseItemSeen = expressAsyncHandler(
  async (req, res, next) => {
    ...
    const itemsIndex = coures.couresItems.findIndex(
      (item) => item._id.toString() === req.params.id
    );
    if (itemsIndex > -1) {
      const cartItem = coures.couresItems[itemsIndex];
      cartItem.quantity = req.body.seen;
      coures.couresItems[itemsIndex] = cartItem;
    } else {
