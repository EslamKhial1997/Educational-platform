...
exports.createCoupon = expressAsyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < numCoupons; i++) {
      const code = `COUPON${i + 1}`; // هنا ممكن تستخدم دالة لتوليد أكواد عشوائية
      const newCoupon = new Coupon({
          code: code,
          discount: discount,
          expirationDate: new Date(Date.now() + 30*24*60*60*1000), // صلاحية لشهر مثلاً
      });
  ...
  try {
      ...
      console.log(`تم إنشاء ${numCoupons} كوبونات بنجاح.`);
  } catch (error) {
      console.error('حدث خطأ أثناء إنشاء الكوبونات:', error);
  }
});
