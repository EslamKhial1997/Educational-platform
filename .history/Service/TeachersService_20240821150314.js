...
exports.updateSpecificPaid = expressAsyncHandler(async (req, res, next) => {
  const teacher = await createTeachersModel.findOneAndUpdate(
    { _id: req.Teacher._id, "paid._id": req.params.paid }, // العثور على المعلم والدفعة المحددة
    {
      $set: { "paid.$.point": req.body.point, "paid.$.paid": true }, // تحديث حقل `point` في الدفعة
    },
    { new: true }
  );
  ...
  teacher.point -= +request.body.point;
  res.status(200).json({
    status: "success",
    data: teacher,
  });
