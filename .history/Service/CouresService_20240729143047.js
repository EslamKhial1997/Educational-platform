...
exports.createCoures = expressAsyncHandler(async (req, res, next) => {
  ...
  if (!coures) {
    ...
    if (couresExists > -1) {
     m.coupon = couponModel.code;
      coures.couresItems = cartItem; const cartItem = coures.couresItems[couresExists];
      cartItem.discount = couponModel.discount;
      cartIte
    } else {
      
      coures.couresItems.push({ lacture, couponModel });
    }
  }
...
const teacherModel = await createTeachersModel.findById(lactureModel.teacher._id)
