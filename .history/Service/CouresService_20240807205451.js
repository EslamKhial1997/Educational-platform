...
console.log(lactureModel.section._id);
const classes = await createClassModel.findById(lactureModel.section._id)
    let coures = await createCouresModel.findOne({ user: req.user._id });
    const { lacture } = req.body;
    ...
    if (!coures && !couponModel) {
      ...
      if (courseExistsIndex === -1) {
        ...
        const courseTeacherIndex = coures.teacher.findIndex(
          (item) =>
            item.teacherID.toString() === lactureModel.teacher._id.toString()
        );
    ...
    const deleteDoc = await createCouponsModel.findByIdAndDelete(
      couponModel._id
    );
    user.ip = serverIp;
