...
exports.createCoures = expressAsyncHandler(async (req, res, next) => {
  ...
  // if (req.body.lacture !== couponModel.lecture._id.toString())
  //   next(
  //     new ApiError(`Coupon :${req.body.coupon} Not Below This Lecture`, 500)
  //   );
...
console.log(lactureModel.teacher);
  const { lacture } = req.body;
  if (!coures) {
    coures = await createCouresModel.create({
      user: req.user._id,
      teacher: [{name:lactureModel.teacher.name , id:lactureModel.teacher._id}],
      couresItems: [
        { lacture,teacher: lactureModel.teacher._id, coupon: couponModel.code, discount: couponModel.discount },
      ],
    ...
    if (couresExists > -1) {
      ...
      teacher= [{name:lactureModel.teacher.name , teacherID:lactureModel.teacher._id}],
