...
exports.allowedTo = (...roles) =>
  ...
  exports.protect = expressAsyncHandler(async (req, res, next) => {
    ...
    // العثور على المستخدم أو المعلم بناءً على الـID المستخرج من التوكن
    const currentUser = await createUsersModel.findById(decoded.userId);
    const currentTeacher = await createTeachersModel.findById(decoded.userId);
    ...
    // التحقق مما إذا قام المستخدم بتغيير كلمة المرور بعد إصدار التوكن (فقط للمستخدمين)
    ...
    // التحقق من دور المستخدم أو المعلم
    const role = currentUser ? currentUser.role : currentTeacher.role;
    ...
    // تعيين المستخدم أو المعلم في الطلب (req) لمزيد من الاستخدام
    req.user = currentUser || currentTeacher;
