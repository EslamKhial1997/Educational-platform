  ...
  expressAsyncHandler(async (req, res, next) => {
    try {
      ...
      for (const key of imageKeys) {
        if (req.body[key] !== undefined) { // تحقق إذا كان الحقل موجودًا في req.body
  ...
  expressAsyncHandler(async (req, res, next) => {
    try {
      ...
      if (
        ...
        (findGallery.picture &&
          req.body.picture &&
          findGallery.avater !== req.body.picture)
      ) {
        const relativePathImage =
          findGallery.avater.split(baseUrl)[1] ||
          findGallery.image.split(baseUrl)[1] ||
          findGallery.picture.split(baseUrl)[1];
        filePathImage(filePath, relativePathImage); // حذف الصورة القديمة
      }
