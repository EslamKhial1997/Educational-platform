...
exports.updateOne = (Model, filePath) =>
  expressAsyncHandler(async (req, res, next) => {
    try {
      ...
      // العثور على الوثيقة بناءً على ID
      ...
      const imageKeys = ["image", "avatar", "picture"];

      for (const key of imageKeys) {
        console.log(findDocument.avater);
        console.log(req.body.key);
        
        if (
          findDocument.key &&
          req.body.key &&
          findDocument.key !== req.body.key
        ) {
          console.log("done");
          ...
          await filePathImage(filePath, relativePathImage); // حذف الصورة القديمة
        }
