...
exports.updateSpecificCourseItemSeen = expressAsyncHandler(
  async (req, res, next) => {
    ...
    if (itemsIndex > -1) {
      ...
      // إذا كانت قيمة seen تساوي 0، يتم حذف العنصر
      if (courseItem.seen === 0) {
        course.couresItems.splice(itemsIndex, 1); // حذف العنصر من القائمة
      } else {
        course.couresItems[itemsIndex] = courseItem; // تحديث العنصر في القائمة
      }
