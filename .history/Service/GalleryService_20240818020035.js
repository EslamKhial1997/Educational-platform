...
exports.updateGallery = expressAsyncHandler(async (req, res, next) => {
  try {
    ...
    // تحديث مسار الصورة إذا تم توفيرها في الطلب
    if (req.body.image) {
      updateDocById.image = req.body.image;
      await updateDocById.save();

      // حذف الصورة القديمة إذا تم تحديثها
      const relativePathAvatar = updateDocById.image.replace(baseUrl, "");
      const filePathAvatar = path.join(__dirname, "../uploads/gallery", relativePathAvatar);
      fsRemove(filePathAvatar);
    }
