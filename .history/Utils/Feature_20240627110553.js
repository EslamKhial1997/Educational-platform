  ...
  Search(modelName) {
    ...
    if (modelName === undefined) {
        if (this.QueryStringApi.keyword) {
            QuerySearch.$or = [
                { coupon.number: { $elemMatch: { $regex: new RegExp(this.QueryStringApi.keyword, "i") } } },
                // Add other fields to search in if needed
            ];
        }
    } else {
        QuerySearch = {
            coupon: { $elemMatch: { $regex: new RegExp(this.QueryStringApi.keyword, "i") } },
        };
    }

    // Add filter by a specific value
